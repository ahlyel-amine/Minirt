...
/*   Updated: 2023/12/16 00:09:42 by aahlyel          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	cone_parcer(char *line, t_data *data)
{
	...
	if (!cordinate_parcer(&line, &cone.cord, INT_MAX, INT_MIN))
		return (ft_putendl_fd_arg(4, ERR, ERR_CN, ERR_CORD), false);
	if (!cordinate_parcer(&line, &cone.normalized, 1, -1))
		return (ft_putendl_fd_arg(4, ERR, ERR_CN, ERR_N), false);
	if (!ft_atod(&line, &cone.half_angle, 180, 0))
		return (ft_putendl_fd_arg(4, ERR, ERR_CN, ERR_HANGLE), false);
	if (!ft_atod(&line, &cone.height, INT_MAX, INT_MIN))
		return (ft_putendl_fd_arg(4, ERR, ERR_CN, ERR_HEIGHT), false);
	line += skip_spaces(line);
	i = color_parcer(line, &cone.clr);
	if (i == -1)
		return (ft_putendl_fd_arg(4, ERR, ERR_CN, ERR_COLOR), false);
